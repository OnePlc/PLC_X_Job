...
class ApiPositionController extends CoreController {
    /**
     * Job Table Object
     *
     * @since 1.0.0
    ...
    public function indexAction() {
        ...
        $aReturn = ['state'=>'success','message'=>'Welcome to onePlace Position API'];
    ...
    /**
     * List all Entities of Positions
     *
     * @return bool - no View File
     * @since 1.0.0
     */
    public function listAction() {
        ...
        # Check license
        if(!$this->checkLicense('jobposition')) {
            $aReturn = ['state'=>'error','message'=>'no valid license for jobposition found'];
            echo json_encode($aReturn);
            return false;
        }
        ...
        foreach($aLangs as $sLoadLang) {
            if(file_exists(__DIR__.'/../../../oneplace-translation/language/'.$sLoadLang.'.mo')) {
                $translator->addTranslationFile('gettext', __DIR__.'/../../../oneplace-translation/language/'.$sLang.'.mo', 'jobposition', $sLoadLang);
            }
        }
        ...
        $aFields = $this->getFormFields('jobposition-single');
        $aFieldsByKey = [];
        ...
        # Get All Position Entities from Database
                        ...
                        switch($oField->type) {
                            case 'multiselect':
                                ...
                                foreach($oTags as $oTag) {
                                    $aTags[] = ['id'=>$oTag->id,'label'=>$translator->translate($oTag->text,'jobposition',$sLang)];
                                }
                            ...
                            case 'select':
                                ...
                                if($oTag) {
                                    if (property_exists($oTag, 'tag_value')) {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->id, 'label' => $translator->translate($oTag->tag_value,'jobposition',$sLang)];
                                    } else {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->getID(), 'label' => $translator->translate($oTag->getLabel(),'jobposition',$sLang)];
                                    }
                                }
                                break;
                            case 'text':
                            case 'date':
                            case 'textarea':
                                $aPublicItem[$oField->fieldkey] = $translator->translate($oItem->getTextField($oField->fieldkey),'jobposition',$sLang);
                                break;
                            default:
                                break;
                        }
    ...
    /**
     * Get a single Entity of Position
     *
     * @return bool - no View File
     * @since 1.0.0
     */
    public function getAction() {
        ...
        try {
            ...
            $aReturn = ['state'=>'error','message'=>'Position not found','oItem'=>[]];
            echo json_encode($aReturn);
            return false;
        }

        # Print Entity
        $aReturn = ['state'=>'success','message'=>'Position found','oItem'=>$oItem];
        echo json_encode($aReturn);
