...
class ApiController extends CoreController {
    ...
    public function listAction() {
        ...
        # Check license
        if(!$this->checkLicense('job')) {
            $aReturn = ['state'=>'error','message'=>'no valid license for job found'];
            echo json_encode($aReturn);
            return false;
        }
        ...
        foreach($aLangs as $sLoadLang) {
            if(file_exists(__DIR__.'/../../../oneplace-translation/language/'.$sLoadLang.'.mo')) {
                $translator->addTranslationFile('gettext', __DIR__.'/../../../oneplace-translation/language/'.$sLang.'.mo', 'job', $sLoadLang);
            }
        }
        ...
        $aFields = $this->getFormFields('job-single');
        $aFieldsByKey = [];
                        ...
                        switch($oField->type) {
                            case 'multiselect':
                                ...
                                foreach($oTags as $oTag) {
                                    $aTags[] = ['id'=>$oTag->id,'label'=>$translator->translate($oTag->text,'job',$sLang)];
                                }
                            ...
                            case 'select':
                                ...
                                if($oTag) {
                                    if (property_exists($oTag, 'tag_value')) {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->id, 'label' => $translator->translate($oTag->tag_value,'job',$sLang)];
                                    } else {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->getID(), 'label' => $translator->translate($oTag->getLabel(),'job',$sLang)];
                                    }
                                }
                                break;
                            case 'text':
                            case 'date':
                            case 'textarea':
                                $aPublicItem[$oField->fieldkey] = $translator->translate($oItem->getTextField($oField->fieldkey),'job',$sLang);
                                break;
                            default:
                                break;
                        }
